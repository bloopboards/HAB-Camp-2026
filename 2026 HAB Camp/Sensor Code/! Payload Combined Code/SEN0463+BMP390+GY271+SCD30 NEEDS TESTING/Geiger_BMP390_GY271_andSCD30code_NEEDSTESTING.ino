...
// Sets starting values for SCD30
float co2 = 0;
float humidity = 0;
...
void setup() {
  ...
  if (myFile) {
    ...
    myFile.println(F("Temperature (*C), Pressure (hPa), Approx. Altitude (meters), CPM, nSv/h, uSv/h, Magnetic field X (mG), Magnetic field Y (mG), Magnetic field Z (mG), Heading (°), Magnitude (mG), CO2 (ppm), Humidity (%)"));
    myFile.close();
