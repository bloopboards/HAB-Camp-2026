...
void setup() {
  ...
  csvFile = SD.open("information.csv", FILE_WRITE);

  // if the file opened okay, write to it:
  if (csvFile) {
    Serial.print("Writing to file...");
    myFile.println("Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
    // close the file:
    myFile.close();
    Serial.println("done.");
  } else {
...
void loop() {
  ...
  if (csvFile) {
    Serial.print("Writing to file...");
    myFile.print(millis()/1000); myFile.print(", ")
    myFile.print(sensors.getTempCByIndex(0)); myFile.print(", ") //(Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
    myFile.print(event.magnetic.x); myFile.print(", ") //(Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
    myFile.print(event.magnetic.y); myFile.print(", ") //(Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
    myFile.print(event.magnetic.z); myFile.print(", ") //(Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
    myFile.print(myUVSensor.getUVA()); myFile.print(", ") //(Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
    myFile.print(myUVSensor.getUVB()); myFile.print(", ") //(Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
    myFile.print(myUVSensor.getUVC()); myFile.print("\n") //(Time (s), Temperature (C), x (uT), y (uT), z (uT), UVA, UVB, UVC");
