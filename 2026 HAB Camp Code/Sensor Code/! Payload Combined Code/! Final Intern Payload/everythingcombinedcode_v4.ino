/*
  ...
  Solar Panel:
  5V -> Common Slot with Resistor and A0 -> A0
  GND -> Common Slot with Resistor and GND -> GND
...
// Sets starting value for DS18B20
float outsidetemp = 0;
...
float solarpower;
float intensity;
const float area = 0.0039875; // area of sensor in m^2
...
int insideheaterPin = 8;
int outsideheaterPin = 4;
int heaterTemp = 100; //heaters start at 10*C

int insideheaterstatus = 0;
int outsideheaterstatus = 0;
...
void setup() {
  ...
  // Setup the compass and initially start heaters off
  Wire.begin();
  digitalWrite(insideheaterPin, LOW);
  digitalWrite(outsideheaterPin, LOW);
  ...
  Serial.println(F("BMP390+SEN0463+GY271+PMSA003I+SCD30+SolarPanel Test"));
  
  // Starts counting with the Geiger counter
  geiger.start();

  // Starts DS18B20 Temp Sensor
  sensors.begin();
  ...
  if (myFile) {
    ...
    myFile.println(F("External Temp (*C), Internal Temp (*C), Pressure (hPa), Approx. Altitude (meters), CPM, nSv/h, uSv/h, Mag. field X (mG), Mag. field Y (mG), Mag. field Z (mG), Heading (°), Magnitude (mG), CO2 (ppm), SCD30 Temp (C), Humidity (%), Particles > 0.3um / 0.1L air, Particles > 0.5um / 0.1L air, Particles > 1.0um / 0.1L air, Particles > 2.5um / 0.1L air, Particles > 5.0um / 0.1L air, Particles > 10 um / 0.1L air, Voltage (V), Power (mW), Solar Power (mW), Intensity (W/m^2), Inside Heater Status, Outside Heater Status, Time (seconds)"));
    myFile.close();
...
void loop() {
  ...
  } else Serial.println("AQI reading success");
  ...
  // If no data is available, it will print whatever the last data available was or zero
  ...
  // BMP390
  Serial.print(insidetemp);     Serial.print(F(", "));
  ...
  Serial.print(SCDtemp); Serial.print(F(", "));
  ...
  // Start Outside Heater
  if (heaterTemp > outsidetemp) { 
    digitalWrite(outsideheaterPin, HIGH); 
    int outsideheaterstatus = 1; }
    else {
      digitalWrite(outsideheaterPin, LOW);
      int outsideheaterstatus = 0;
    }
  ...
  // Start Inside Heater
  if (heaterTemp > insidetemp) {
    digitalWrite(insideheaterPin, HIGH); 
    int insideheaterstatus = 1; }
    else {
      digitalWrite(insideheaterPin, LOW);
      int insideheaterstatus = 0;
    }
  ...
  Serial.print(voltage); Serial.print(F(", "));
  Serial.print(power * 1000); Serial.print(F(", ")); 
  Serial.print(solarpower * 1000); Serial.print(F(", "));
  Serial.print(intensity); Serial.print(F(", "));

  // Timer
  myTime = millis();
  Serial.print(myTime/1000); Serial.print(F(", "));
  ...
  if (myFile) {
    ...
    // BMP390
    myFile.print(insidetemp);     myFile.print(",");
    ...
    myFile.print(SCDtemp); myFile.print(",");
    ...
    myFile.print(power * 1000); myFile.print(", ");
    myFile.print(solarpower * 1000); myFile.print(", ");
    myFile.print(intensity); myFile.print(", ");

    // Heater Status
    myFile.print(insideheaterstatus); myFile.print(",");
    myFile.print(outsideheaterstatus); myFile.print(",");

    // Timer
    myFile.print(myTime/1000);
